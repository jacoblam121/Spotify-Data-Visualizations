#!/usr/bin/env python3
"""
ENHANCED IMMEDIATE FIX - Complete Global Variable Capture + Missing Config Variables

This script fixes the NIGHTINGALE_TITLE_FONT_SIZE issue by including configuration-loaded
variables that weren't captured by the original introspective approach.

FIXES:
1. ✅ Captures ALL global variables automatically
2. ✅ Adds missing NIGHTINGALE configuration variables manually
3. ✅ Fixes silent failure issue with explicit error handling  
4. ✅ Makes frame files actually get saved
5. ✅ Gets your visualizations working immediately

APPROACH: Enhanced introspective capture + explicit missing variable addition
"""

import os
import shutil

def create_enhanced_production_fix():
    """Create enhanced production fix that includes missing config variables"""
...
'''
    ...
    with open("/home/jacob/Spotify-Data-Visualizations/main_animator_ENHANCED_FIX.py", 'w') as f:
        f.write(content)
    
    print("✅ Created main_animator_ENHANCED_FIX.py")

def update_main_animator_to_enhanced():
    """Update main_animator.py to use the enhanced fix"""
    ...
    # Replace the import
    old_import = "from main_animator_COMPLETE_FIX import replace_broken_parallel_processing_COMPLETE"
    new_import = "from main_animator_ENHANCED_FIX import replace_broken_parallel_processing_ENHANCED"
    
    if old_import in content:
        content = content.replace(old_import, new_import)
        print("✅ Updated import to use enhanced fix")
    else:
        # Try the original import
        old_import_orig = "from main_animator_PRODUCTION_FIX import replace_broken_parallel_processing_PRODUCTION"
        if old_import_orig in content:
            content = content.replace(old_import_orig, new_import)
            print("✅ Updated import from original to enhanced fix")
    
    # Replace the function call
    old_calls = [
        "replace_broken_parallel_processing_COMPLETE(",
        "replace_broken_parallel_processing_PRODUCTION("
    ]
    new_call = "replace_broken_parallel_processing_ENHANCED("
    
    for old_call in old_calls:
        if old_call in content:
            content = content.replace(old_call, new_call)
            print(f"✅ Updated function call {old_call} to use enhanced fix")
    
    with open(main_file, 'w') as f:
        f.write(content)

def main():
    """Apply the enhanced immediate fix"""
    print("🎯 ENHANCED IMMEDIATE FIX - Missing Configuration Variables")
    print("=" * 60)
    print("Fixing NIGHTINGALE_TITLE_FONT_SIZE and other missing config variables")
    print()
    
    # Step 1: Create enhanced fix
    print("1. Creating enhanced production fix...")
    create_enhanced_production_fix()
    
    # Step 2: Update main_animator.py
    print("\\n2. Updating main_animator.py imports...")
    update_main_animator_to_enhanced()
    
    print("\\n🎉 ENHANCED IMMEDIATE FIX APPLIED!")
    print("=" * 60)
    print("✅ Complete global variable capture implemented")
    print("✅ Missing NIGHTINGALE config variables manually added")
    print("✅ NIGHTINGALE_TITLE_FONT_SIZE = 22 (from configurations.txt)")
    ...
    print("- All NIGHTINGALE variables properly available") 
